// crates/vectorprime-model-ir/src/lib.rs
//
// Model IR Analyzer for VectorPrime.
//
...
// `vectorprime-bindings` to populate [`vectorprime_core::ModelInfo::param_count`]
// before the optimization pipeline runs.
//
// Usage:
//   Called from `vectorprime-bindings` via `parse_model(path)`.  Errors are
...
use vectorprime_core::ModelFormat;
