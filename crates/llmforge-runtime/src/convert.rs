...
use llmforge_core::RuntimeError;

const GGUF_TO_ONNX_RUNNER: &str = "python/llmforge/gguf_to_onnx_runner.py";
const ONNX_TO_GGUF_RUNNER: &str = "python/llmforge/onnx_to_gguf_runner.py";
...
pub fn gguf_to_onnx(input: &Path, output: &Path) -> Result<PathBuf> {
    ...
    let runner = find_runner(GGUF_TO_ONNX_RUNNER).ok_or_else(|| {
        anyhow::anyhow!("gguf_to_onnx_runner.py not found — ensure llmforge is properly installed")
    })?;
...
pub fn onnx_to_gguf(input: &Path, output: &Path) -> Result<PathBuf> {
    ...
    let runner = find_runner(ONNX_TO_GGUF_RUNNER).ok_or_else(|| {
        anyhow::anyhow!("onnx_to_gguf_runner.py not found — ensure llmforge is properly installed")
    })?;
